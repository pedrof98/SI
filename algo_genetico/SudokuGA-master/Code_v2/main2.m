...
for i=1:num_epochs
    ...
    if(i > numSlopePoints)
        ...
        if(slope <= slopeThreshold)
            ...
            disp(['Resetting Population ...(',num2str(resetCount+1),')'])
            ...
            if(resetCount<resetLimit)
                ...
                avengers(resetCount) = bestChild;
                avengersFitnessMat{resetCount} = findFitness(cell2mat(bestChild),givens);
                avengersNetFitness(resetCount) = sum(sum(cell2mat(avengersFitnessMat)));
                for j=1:pop_size
                    sudoku_pop{j} = initializeSudoku(sudoku, givens);
                    fitness_matrix{j} = findFitness(sudoku_pop{j},givens);
                    net_fitness(j) = sum(fitness_matrix{j}(:));
            ...
            elseif(resetCount==resetLimit)
                resetCount = resetCount + 1;
                disp('Assemble the Avengers !');
                sudoku_pop = avengers;
                fitness_matrix = avengersFitnessMat;
                net_fitness = avengersNetFitness;
            else
                childFitnessMat = findFitness(cell2mat(bestChild),givens);
                childNetFitness = sum(sum(childFitnessMat));
                [~,I] = sort(avengersNetFitness,'descend');
                
                if(avengersNetFitness(I(1))>=childNetFitness)
                    avengers(I(1)) = bestChild;
                    avengersFitnessMat{I(1)} = childFitnessMat;
                    avengersNetFitness(I(1)) = childNetFitness;
                end
                sudoku_pop = avengers;
                fitness_matrix = avengersFitnessMat;
                net_fitness = avengersNetFitness;
